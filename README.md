...
- [超链接RESTful API设计模式](#超链接restful-api设计模式)
  - [0、前言](#0前言)
  - [1、域名](#1域名)
  - [2、版本（Versioning）](#2版本versioning)
  - [3、路径（Endpoint）](#3路径endpoint)
  - [4、HTTP动词](#4http动词)
  - [5、过滤信息（Filtering）](#5过滤信息filtering)
  - [6、状态码（Status Codes）](#6状态码status-codes)
  - [7、错误处理（Error handling）](#7错误处理error-handling)
  - [8、返回结果](#8返回结果)
  - [9、超媒体](#9超媒体)
  - [10、状态转换(HATEOAS)](#10状态转换hateoas)
  - [11、API入口点](#11api入口点)
  - [12、为什么这样做有用，缺点是什么？](#12为什么这样做有用缺点是什么)
    - [这些观点都很好，但是不利之处是什么？](#这些观点都很好但是不利之处是什么)
  - [13、RESTful API是无状态的，那么身份认证是不是RESTful](#13restful-api是无状态的那么身份认证是不是restful)
    - [会话状态](#会话状态)
    - [无状态约束](#无状态约束)
    - [认证和授权](#认证和授权)
    - [基于状态的Web服务](#基于状态的web服务)
    - [基于无状态的Web服务](#基于无状态的web服务)
    - [总结两者的区别](#总结两者的区别)
  - [14、RESTful API的安全问题](#14restful-api的安全问题)
  - [15、参考文献](#15参考文献)

## 0、前言

掌握DRF框架需要读者对django框架有一定的使用经验，并查阅过django官方文档的资料。例如，django类视图，认证
，模型，以及admin的定制，不然读者可能会产生疑惑，当您使用DRF的时候。

掌握django和DRF是为了提高程序员在开发web程序和restful api应用的工作效率。谈到这点有必要讲一下flask框架
，笔者曾经也是认为django太复杂，不自由，但是自从使用flask框架编写了一个大型网站之后就彻底被改变了这种看法
，我大概花了1年时间编写这个像屎山一样的项目，直到这个项目完成之后的某些天我遇到一些bug，但是我又不想再改屎山
，于是我准备重构的时候，我想到了试试django，当我对django初步掌握之后，然后又开始接触到DRF，接触DRF之后，
我发现之前用flask项目的大多数代码在django和DRF里面，如果我当初是用这两个框架编写的话，意味着我可以不用写
很多很多代码，这一点让我感到非常后悔。我怎么当初不用django和DRF。

由于需要提高工作效率从而赚更多的钱，也同时让我疲惫的身躯不在让这些技术含量低的业务逻辑所折磨，愈发让我感到，
程序员使用框架来工作，使用底层api来提高编程基础的重要性。

由于大多数情况下，需要在规定的时间内，高效的完成工作，还要保护自己的身体不因工作而雷倒，更加坚定了我熟练
掌握这两个框架的决心。

但是在掌握DRF和django之前，还是很有必要掌握**超链接RESTful设计模式**。
